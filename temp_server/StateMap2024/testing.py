...
states_without_data_but_republican = []
states_without_data_but_democratic = []
states_without_data_no_senate_race = ['AL', 'AK', 'AR', 'CO', 'GA', 'ID', 'IL', 'IA', 'KS', 'KY', 'LA', 'ME', 'NH', 'NC', 'OK', 'OR', 'SC', 'SD', 'VT', 'ME2', 'NE2']
...
def US_oddsfinder(state,race,cand_FULL_name,cand_last_name_PLUS_bid_PLUS_race,cand_last_name_PLUS_ask_PLUS_race,cand_last_name_PLUS_odds_PLUS_race):
    ...
    with open ("temp_server/StateMap2024-2/Betfarer/price_log_"+str(state)+".txt", "r") as myfile:
...
for number in range(0,52):
    try:
        ...
        if eval("odds_sorted"+str(state)+"[0][0]") == str(state+'_DEM'):
            ...
            exec("html_attribute_" + str(state) + " = \'<font color=#{0}>{1}%</font> vs <font color=#{2}>{3}%</font></b><span style=\"font-size: 14pt;\"><br><font color=#{0}>{4}% in last day. Electoral votes: {5}</font></b>\'.format(" +
        ...
        fp = open("temp_server/StateMap2024-2/Betfarer/price_log_"+str(state)+".txt",'a')
        ...
        print ""
        print "Waiting for 10 seconds before trying the next state."
        time.sleep(10)
    except Exception as e:
        states_without_data_no_senate_race.append(state)
        ...
        if state in states_without_data_no_senate_race:
            exec("color_"+str(state)+"= 'ffffff'")
            exec("html_attribute_" + str(state) + " = \'No senate race this year</b><span style=\"font-size: 14pt;\"><br><b>Current senate is <font color=#0000ff>50</font>-<font color=#ff0000>50</font></b></span>\'")
        print "No data for "+str(state)+". Trying the next one..."
        print "========== END of "+str(state)+" ================"
        print ""

        pass
print states_without_data_no_senate_race
...
<div id="selected-state"><span><b><u>Senate Seats 2024</u> (estimate: <font color=#0000ff>'''+str(DEM_votes)+'''</font>-<font color=#ff0000>'''+str(REP_votes)+'''</font>. Hover over states)</b><span style="font-size: 14pt;"><br><b>Current senate is <font color=#0000ff>50</font>-<font color=#ff0000>50</font></b></span></span></div></div>
<div id="map"></div>
...
<font color=#000000>Last updated: '''+str(now)+'''<br>
...
#     ftp.storbinary(str("STOR files/StateMap2024-2.html"), converted_DEM_HTML_string)
#     print "MIRROR ElectionBettingOdds.com... Success! (Unless immediately followed by error message)"

# ##### FTP! :) #####
# print "writing to server"
# fg = open('/var/www/predictionmarketodds.com/public_html/StateMap2024-2.html','w')
...
# print "writing to server"
# fg = open('/var/www/178.62.65.243/public_html/StateMap2024-2.html','w')
# fg.write(str(finalwinner_HTML_string))
# fg.close()
# print "Mirror 178.62.65.243... Also Success! (Unless immediately followed by error message)"

# print "writing to server"
# fg = open('/var/www/electionbettingodds.com/public_html/StateMap2024-2.html','w')
# fg.write(str(finalwinner_HTML_string))
# fg.close()
...
converted_DEM_HTML_string = io.BytesIO(finalwinner_HTML_string)
ftp.storbinary(str("STOR public_html/StateMap2024-2.html"), converted_DEM_HTML_string)
