...
html_title = "StateMap2024.html"

category = "presidential" # Added: presidential or us-senate(not yet configured for senate)
ST_counter = 0
...
votes_dict = (9,3,11,6,54,10,7,3,30,16,4,4,19,11,6,6,8,8,3,10,11,15,10,6,10,4,4,6,4,14,5,28,16,3,17,7,8,19,4,9,3,11,40,6,3,13,12,4,10,3,1,1,)
final_dict = {}
Betfair_dummy = 0
...
def US_oddsfinder(state,race,cand_FULL_name,cand_last_name_PLUS_bid_PLUS_race,cand_last_name_PLUS_ask_PLUS_race,cand_last_name_PLUS_odds_PLUS_race):
    ...
    predictit_state_urls = [
        ...
        "https://polymarket.com/event/"+str(url)+"-"+str(category)+"-election-winner-2024"
    ]
    time.sleep(3)
    #### BETFAIR HERE

    with open ("/python/StateMap2024/Betfarer/price_log_"+str(state)+".txt", "r") as myfile:
    ...
    ###US_PREDICT_IT###DEM
    # Attempt to fetch HTML content from PredictIt state URLs
    html_content = None
...
for number in range(0,52):
    ...
    key_1 = "***data_counter***1"#+str(ST_data_counter-1) #-int(round((12*24*7),0 "116" "116" #
    key_2 = "***data_counter***2"#+str(ST_data_counter) #-int(round((12*24*7),0 "117" "117" # (oldest possible)
    print key_1
    print key_2
    ...
    fp = open("/python/StateMap2024/Betfarer/price_log_"+str(state)+".txt",'a')
...
break;
case 'ME2':            
description = '<b>Maine (2nd Congressional District): <font color=#'''+str(first_color_to_display_ME2)+str(first_result_to_display_ME2)+'''%</font> vs <font color=#'''+str(second_color_to_display_ME2)+str(second_result_to_display_ME2)+'''%</font></b><span style="font-size: 14pt;"><br><font color=#'''+str(first_color_to_display_ME2)+'''</font>'''+str(change_to_display_ME2)+'''% in last day. Electoral votes: '''+str(votes_dict[50])+'''</font></b>';
break;
case 'MD':            
...
break;
case 'NE2':            
description = '<b>Nebraska (2nd Congressional District): <font color=#'''+str(first_color_to_display_NE2)+str(first_result_to_display_NE2)+'''%</font> vs <font color=#'''+str(second_color_to_display_NE2)+str(second_result_to_display_NE2)+'''%</font></b><span style="font-size: 14pt;"><br><font color=#'''+str(first_color_to_display_NE2)+'''</font>'''+str(change_to_display_NE2)+'''% in last day. Electoral votes: '''+str(votes_dict[51])+'''</font></b>';
break;
case 'NV':            
...
Over $1M bet. State betting is from <a href="https://www.polymarket.com/">Polymarket</a>. Others to be added if liquid state market available<br></span>
</html>
