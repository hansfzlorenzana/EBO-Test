...
### START OF CONFIG ###
category = "presidential" # Added: presidential or us-senate
year = "2024"
folder_name = "StateMap2024" # This is the folder path; also the name of the html file - .html extension is automatically added
page_title = "Electoral Votes 2024" # This is the title displayed on the page
...
states_without_data_no_race = [] # no data and no race
...
def US_oddsfinder(state,race,cand_FULL_name,cand_last_name_PLUS_bid_PLUS_race,cand_last_name_PLUS_ask_PLUS_race,cand_last_name_PLUS_odds_PLUS_race):
    ...
    with open ("/python//"+str(folder_name)+"/Betfarer/price_log_"+str(state)+".txt", "r") as myfile:
...
for number in range(0,52):
    try:
        ...
        fp = open("/python//"+str(folder_name)+"/Betfarer/price_log_"+str(state)+".txt",'a')
...
<div id="selected-state"><span><b><u>'''+str(page_title)+'''</u> (estimate: <font color=#0000ff>'''+str(DEM_votes)+'''</font>-<font color=#ff0000>'''+str(REP_votes)+'''</font>. Hover over states)</b><span style="font-size: 14pt;"><br><b>270 electoral votes needed to win</b></span></span></div></div>
<div id="map"></div>
...
break;}      
    ...
    mouseout: function(event, data) {
      $('#selected-state > span').html('<b><u>'''+str(page_title)+'''</u> (estimate: <font color=#0000ff>'''+str(DEM_votes)+'''</font>-<font color=#ff0000>'''+str(REP_votes)+'''</font>. Hover over states)</b><span style="font-size: 14pt;"><br><b>270 electoral votes needed to win</b></span>').css({"font-weight": "normal", "color": "#000000"});
    },
