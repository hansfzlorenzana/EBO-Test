...
if 'MARKET_POLYMARKET' not in locals():
    MARKET_POLYMARKET = "Polymarket_Old"
if 'MARKET_POLYMARKET_CLOB' not in locals():
    MARKET_POLYMARKET_CLOB = "Polymarket_Clob"
...
if 'Polymarket_manual_entry' in locals():
    manual_entry_by_market[MARKET_POLYMARKET] = Polymarket_manual_entry
if 'Polymarket_clob_manual_entry' in locals():
    manual_entry_by_market[MARKET_POLYMARKET_CLOB] = Polymarket_clob_manual_entry
...
def scrape_polymarket():
    ...
    try:
        ...
        polymarket_id_query_fields = ['id=' + str(pm_id) for pm_id in pm_ids]
        polymarket_query_string = '?' + '&'.join(polymarket_id_query_fields)
        print "12345"
        polymarket_url = 'http://strapi-matic.poly.market/markets/' + polymarket_query_string
        ...
        polymarket_contracts_by_id = {resp_entry['id']: resp_entry for resp_entry in resp_data}
    ...
    return scrape_failed, polymarket_contracts_by_id

def scrape_polymarket_clob():
    scrape_failed = False

    # Scraping Polymarket Clob API
    polymarket_clob_contracts = {}
    try:
        for (cand_FULL_name, _, _) in Polymarket_clob_manual_entry:
            if cand_FULL_name:
                outcome_token_id, condition_id = cand_FULL_name

                price_url_base = 'https://clob.polymarket.com/price'

                bid_query_fields = '?side=buy&token_id={}'.format(str(outcome_token_id))
                ask_query_fields = '?side=sell&token_id={}'.format(str(outcome_token_id))

                def get_json(polymarket_url):
                    if USE_CACHED_RESPONSES:
                        polymarket_raw = open('cached/polymarket_raw', 'r').read()
                    else:
                        polymarket_raw = requests.get(polymarket_url, verify=False).content
                ...
                # NOTE: cand_FULL_name is a tuple of two strings. Tuples can be
                # used as dictionary keys.
                polymarket_clob_contracts[cand_FULL_name] = {
                    'bid':  get_json(price_url_base + bid_query_fields),
                    'ask': get_json(price_url_base + ask_query_fields),
                }

    except Exception as e:
        warning_message = "Error scraping Polymarket Clob API (may try to recover with other markets)"
    ...
    return scrape_failed, polymarket_clob_contracts
...
###### Polymarket ######
def parse_polymarket_odds(polymarket_contracts_by_id, cand_FULL_name):
    # In the case of Polymarket, we will denote skipping a candidate with an empty string or a None
    ...
    if pm_id in polymarket_contracts_by_id and pm_outcome in polymarket_contracts_by_id[pm_id]['outcomes']:
        ...
        outcome_price = float(polymarket_contracts_by_id[pm_id]['outcomePrices'][outcome_index])

        POLYMARKET_FEE_FACTOR = 1
        prepare_number = lambda n: float(round(max(0, min(1, n)), 2))

        Candidate_ask = prepare_number(outcome_price * POLYMARKET_FEE_FACTOR)
        Candidate_bid = prepare_number(1 - (1 - outcome_price) * POLYMARKET_FEE_FACTOR)
        Candidate_odds = (Candidate_bid+Candidate_ask)/2
...
###### Polymarket Clob API ######
def parse_polymarket_clob_odds(polymarket_clob_contracts, cand_FULL_name):
    # In the case of Polymarket Clob API, we will denote skipping a candidate with an empty string or a None
    outcome_token_id, condition_id = cand_FULL_name
    if cand_FULL_name not in polymarket_clob_contracts:
        return {
            'Candidate_found': False,
        }

    # TODO - handle odd cases like low liquidity like the other markets?
    Candidate_ask = float(polymarket_clob_contracts[cand_FULL_name]['ask']['price'])
    Candidate_bid = float(polymarket_clob_contracts[cand_FULL_name]['bid']['price'])
    Candidate_odds = (Candidate_bid+Candidate_ask)/2
...
def parse_polymarket_new_odds(polymarket_contracts_by_id, cand_FULL_name):
    # In the case of Polymarket Clob API, we will denote skipping a candidate with an empty string or a None
    pm_id, pm_outcome = cand_FULL_name
    if pm_id not in polymarket_contracts_by_id:
        return {
            'Candidate_found': False,
        }
...
### Polymarket ###
def parse_polymarket_volume(polymarket_contracts_by_id):
    # NOTE - this gathers volume from all of the ids mentioned.
    # There may be more than one candidate per id.
    return int(sum(float(contract['volume']) for contract in polymarket_contracts_by_id.values()))

### Polymarket Clob API ###
def parse_polymarket_clob_volume(polymarket_clob_contracts):
    # NOTE - this gathers volume from all of the candidates mentioned.
    # for contract in list(polymarket_clob_contracts.values()):
    #     print(contract)
    return int(sum(float(contract['bid']['price']) for contract in list(polymarket_clob_contracts.values())))
...
if MARKET_POLYMARKET in USED_MARKETS:
    print("testme1")
    scrape_failures[MARKET_POLYMARKET], polymarket_contracts_by_id = scrape_polymarket()
    print("testme2")
if MARKET_POLYMARKET_CLOB in USED_MARKETS:
    scrape_failures[MARKET_POLYMARKET_CLOB], polymarket_clob_contracts = scrape_polymarket_clob()
    ...
    for current_market_index, current_market in enumerate(USED_MARKETS):
        ...
        def oddsfinder(current_contract_index, cand_FULL_name, cand_last_name):
            ...
            if cand_FULL_name:
                ...
                if current_market == MARKET_POLYMARKET:
                    Candidate_parsed_data = parse_polymarket_odds(polymarket_contracts_by_id, cand_FULL_name)

                if current_market == MARKET_POLYMARKET_CLOB:
                    Candidate_parsed_data = parse_polymarket_clob_odds(polymarket_clob_contracts, cand_FULL_name)
        ...
        if current_market == MARKET_POLYMARKET:
            market_volumes[MARKET_POLYMARKET] = parse_polymarket_volume(polymarket_contracts_by_id)

        if current_market == MARKET_POLYMARKET_CLOB:
            market_volumes[MARKET_POLYMARKET_CLOB] = parse_polymarket_clob_volume(polymarket_clob_contracts)
        ...
        if current_market_index == num_markets - 1:
            ...
            MARKET_TOOLTIP_DISPLAY = {
                ...
                MARKET_POLYMARKET: "Polymarket-Old " + GLOBE_EMOJI,
                MARKET_POLYMARKET_CLOB: "Polymarket-Clob " + GLOBE_EMOJI,
