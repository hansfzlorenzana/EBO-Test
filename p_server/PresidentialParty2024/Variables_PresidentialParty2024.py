...
try:
    ...
    print ("Error: Need a valid temp_server/eboconfig.py file. See temp_server/eboconfig.py.example for reference.")
    raise
...
########################
my_ticker = "PresidentialParty2024" #MUST be in the title of "price_log" file
my_html_address = "PresidentialParty2024" #program automatically inserts ".html"
my_root = "PresidentialParty2024"# no slashes before or after

my_Smarkets_market_name = "2024 winning party" #INSTEAD of URL. Case sensitive
my_Betfair_URL = "https://ero.betfair.ro/www/sports/exchange/readonly/v1/bymarket?_ak=nzIFcwyWhrlwYMrh&alt=json&currencyCode=USD&locale=en_GB&marketIds=1.178176964&rollupLimit=25&rollupModel=STAKE&types=MARKET_STATE,MARKET_RATES,MARKET_DESCRIPTION,EVENT,RUNNER_DESCRIPTION,RUNNER_STATE,RUNNER_EXCHANGE_PRICES_BEST,RUNNER_METADATA,MARKET_LICENCE,MARKET_LINE_RANGE_INFO"
my_PredictIt_URL = "https://www.predictit.org/api/marketdata/markets/6867"
my_FTX_URL = "https://ftx.com/api/markets/"#<- this one never needs specific link; instead put mkt in 1st col in FTX_manual_entry
my_Polymarket_URL = "https://polymarket.com/event/which-party-will-win-the-2024-united-states-presidential-election"

column_title = "Presidency 2024 (by party)"

time_created = "Since 7am Jan 3"
...
race_description = """Bets on which party will control the Presidency after Nov 5, 2024 elections"""

### CHARTING ENTRIES
custom_chart_labels = ("DEM","REP")
# below always starts with 0. make string empty for defaults
chart_label_ordering = "var viewColumns = [0, 2, 1];view.setColumns(viewColumns);"
#!# NOTE: if you move a label up, must move color with it. Color is stuck to position.
# Make string empty for defaults
chart_colors = "colors: ['red', 'blue'],"
...
try:
    with open ("temp_server/FTX_Universal_Machine.py", "r") as myfile:
...
### HANDLE ALL ERRORS
    ...
    try:
        ...
        g = open('temp_server/'+my_root+'/error_email'+my_ticker+'.txt','w')
        g.write(str(e))
        g = open('temp_server/'+my_root+'/error_email'+my_ticker+'.txt','r')
